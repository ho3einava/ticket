  ...
  return (
    <div>
      <div className=" flex relative w-[80rem] h-[20rem] bg-gray-200 rounded-[5px]">
        <h3 className="font-bold text-xl absolute top-[2rem] right-[5rem]">انتخاب صندلی:</h3>
        ...
        <div className="absolute left-[1rem] top-[3rem] border-2 border-solid border-black rounded-[1rem] w-[40rem] h-[15rem] bg-white ">
          ...
          <div className="absolute left-[7rem] top-[1rem] flex flex-wrap gap-2 w-[13rem] py-2 justify-center">
            {data &&  data.result.slice(0,8).map((e) => (
              <SeatNum data ={e}/>
            ))}


          </div>
          <div className="absolute right-[2rem] top-[1rem] flex flex-wrap gap-2 w-[15rem] py-2 justify-center ">
          {data &&  data.result.slice(8,16).map((e) => (
              <SeatNum data ={e}/>
            ))}
          </div>
          <div className="absolute right-[1rem] bottom-[0.5rem] flex flex-wrap gap-1 w-[35rem] py-2  justify-center">
          {data &&  data.result.slice(16,26).map((e) => (
              <SeatNum data ={e}/>
            ))}
          </div>
        </div>
