  ...
  return (
    <div>
      ...
      <div className="bg-gray-100 w-[60rem] h-[200px] rounded relative mt-[1rem] mr-[5rem]">
        <img
          src={data.logoPath}
          className="absolute right-[2rem] top-[3rem] w-10 h-10 rounded-full"
        />
        <div className="absolute top-[3rem] right-[15rem] font-bold">
          {data.companyName}
        </div>
        <div className="absolute top-[8rem] right-[2rem] text-3xl font-bold">
          {data.departureTime}
        </div>
        <div className="absolute top-[3rem] right-[6rem] flex flex-wrap bg-gray-200 w-[7rem] h-10  items-center justify-center  rounded-full text-xs">
          {data.busType}
        </div>
        ...
        <div className="flex flex-col text-center gap-[2rem]  absolute top-[2rem] left-[1rem] ">
          <div className="mt-1 ">
            <Link
              type="button"
              className="border-solid border-2 border-blue-700 text-blue-700 text-sm px-[7rem] py-[1rem]    rounded-lg mb-3"
              to="/tickets"
            >
              تغییر بلیط
            </Link>
